        ...
        <a v-bind:href="`/cardetail?cardetailpriceid=${item._id}`">
          <div v-bind:id="`carousel-img-${i}`" class="carousel slide" data-ride="carousel">
            <ol class="carousel-indicators">
              <li
                ...
                v-for="(subitem,index) in item.priceByCarType[0].carImages"
          ...
          <div class="card-body w-100 text-left p-1">
            <p class="card-text text-uppercase p-0 m-0 text-sm hidden-outof-text">
              <span class="w-50 hidden-outof-text">{{item.priceByCarType[0].carTypeName}}</span>
            </p>
            <p class="card-text font-weight-bolder p-0 m-0 hidden-outof-text">
              {{item.fromLocation}}
...
export default {
  ...
  methods: {
    ...
    async initial() {
      ...
      var response = await CarPriceService.getAllCarPrice();
