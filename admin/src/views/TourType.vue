<template>
  <div>
              ...
              <v-container grid-list-xl>
                <v-layout wrap>
                  <v-flex xs12 sm6 md4>
                    <v-text-field
                      required
                      :rules="[() => editedItem.tourTypeCode.length > 0 || 'Required field']"
                      v-model="editedItem.tourTypeCode"
                      label="tourTypeCode"
                    ></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm6 md4>
                    <v-text-field v-model="editedItem.tourTypeName" label="tourTypeName"></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm6 md4>
                    <v-select
    ...
    <v-data-table :headers="headers" :items="tourType" :search="search" class="elevation-1">
  ...
  methods: {
    ...
    deleteItem(item) {
      confirm("Are you sure you want to delete this item?") &&
        AXIOS.delete(apiIP + "/tourtype/" + item._id)
          .then(response => {
