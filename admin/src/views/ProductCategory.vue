<template>
  <div>
    <v-toolbar flat color="white">
      <v-toolbar-title>CATEGORY CRUD</v-toolbar-title>
                ...
                <v-layout wrap>
                 <v-flex xs12 sm6 md4>
                  ...
                  <v-flex xs12 sm6 md4>
                    <v-text-field
                      required
                      :rules="[() => editedItem.productCategoryCode.length > 0 || 'Required field']"
                      v-model="editedItem.productCategoryCode"
                      label="productCategoryCode"
                    ></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm6 md4>
                    <v-text-field v-model="editedItem.productCategoryName" label="productCategoryName"></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm6 md4>
                ...
                <v-layout wrap>
                   <v-flex xs12 sm12 md4>
                    ...
                    <file-upload
                      ...
                      v-bind:routerPath="apiIP+'/upload/product/category'"
                      :title="`Upload High Quality`"
                    ></file-upload>
                  </v-flex>
                  <v-flex xs12 sm12 md8>
                    <ImageListComponent
                      :data="editedItem.productCategoryImages"
                  ...
                  <v-flex xs12 sm12 md4>
                    ...
                    <file-upload
                      ...
                      v-bind:routerPath="apiIP+'/upload/product/category/webmp'"
                      :title="`Upload Webp Image`"
                    ></file-upload>
                  </v-flex>
                  <v-flex xs12 sm12 md8>
                    <ImageListComponent
                      :data="editedItem.productCategoryImagesWebp"
    ...
    <v-data-table :headers="headers" :items="productCategory" :search="search" class="elevation-1">
      <template v-slot:items="props">
        <tr class="whitespace-nowrap">
          ...
          <td>{{ props.item.productCategoryCode }}</td>
          <td>{{ props.item.productCategoryName }}</td>
    ...
    <v-snackbar v-model="snackbar.snackbar">
      ...
      <v-btn absolute dark fab bottom right small color="pink">
        <download-excel :data="productCategory" name= "productCategory.xls">
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      {
        text: "productCategoryCode",
        value: "productCategoryCode"
      },
      { text: "productCategoryName", value: "productCategoryName" },
  ...
  methods: {
    initialize() {
      AXIOS.get(apiIP + "/productcategory/", { crossdomain: true })
        .then(response => {
    ...
    deleteItem(item) {
      confirm("Are you sure you want to delete this item?") &&
        AXIOS.delete(apiIP + "/productcategory/" + item._id)
          .then(response => {
    ...
    deleteImage(image) {
      this.editedItem.productCategoryImages.forEach(function(item, index, object) {
    ...
    deleteImageWebp() {
      this.editedItem.productCategoryImagesWebp.forEach(function(item, index, object) {
        if (image.fileName == item.fileName) {
          object.splice(index, 1);
        }
      });
    ...
    save() {
      if (this.uploadImg.length > 0) {
        this.uploadImg.forEach(element => {
          this.editedItem.productCategoryImages.push(element);
        });
      }
      if (this.uploadImgWebp.length > 0) {
        this.uploadImgWebp.forEach(element => {
          this.editedItem.productCategoryImagesWebp.push(element);
        });
      }
      ...
      if (this.$refs.form.validate()) {
        if (this.editedIndex > -1) {
          AXIOS.post(apiIP + "/productcategory/update/"+ this.editId, this.editedItem)
            .then(response => {
            })
            .catch(function(error) {})
            .finally(function() {});
        } else {
          AXIOS.post(apiIP + "/productcategory/insert", this.editedItem)
            .then(response => {
            })
            .catch(function(error) {})
            .finally(function() {});
